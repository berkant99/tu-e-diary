...
searchBar.onkeyup = () => {
    ...
    xml.open("POST", "/e-diary/teacher/serverControllers/chatControllers/search.php", true);
    xml.onload = () => {
...
setInterval(() => {
    let xml = new XMLHttpRequest();
    xml.open("POST", "/e-diary/teacher/serverControllers/chatControllers/userList.php", true);
    xml.onload = () => {
        if (xml.readyState === XMLHttpRequest.DONE) {
            if (xml.status === 200) {
                let data = xml.response;
...
function sendUserLinkId(linkId) {
    let xml = new XMLHttpRequest();
    xml.open("POST", "/e-diary/teacher/serverControllers/chatControllers/userToChat.php", true);
    xml.onload = () => {
        if (xml.readyState === XMLHttpRequest.DONE) {
            if (xml.status === 200) {
                let data = xml.response;
...
function setStatus(status, id) {
    let xml = new XMLHttpRequest();
    xml.open("POST", "/e-diary/teacher/serverControllers/chatControllers/getStatus.php", true);
    xml.onload = () => {
        if (xml.readyState === XMLHttpRequest.DONE) {
            if (xml.status === 200) {
                let data = xml.response;
...
function getChat(id, chatBox) {
        let xml = new XMLHttpRequest();
        xml.open("POST", "/e-diary/teacher/serverControllers/chatControllers/getChat.php", true);
        xml.onload = () => {
            if (xml.readyState === XMLHttpRequest.DONE) {
                if (xml.status === 200) {
                    let data = xml.response;
...
function insertChat(btn, form, chatBox, inputField, incomingID) {
    ...
    btn.onclick = () => {
        ...
        xhr.open("POST", "/e-diary/teacher/serverControllers/chatControllers/insertChat.php", true);
        xhr.onload = () => {

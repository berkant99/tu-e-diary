...
searchBar.onkeyup = () => {
    ...
    xml.open("POST", "/e-diary/student/serverControllers/teacherControllers/search.php", true);
    xml.onload = () => {
...
function sendUserLinkId(linkId) {
    let xml = new XMLHttpRequest();
    xml.open("POST", "/e-diary/student/serverControllers/teacherControllers/showTchrInfo.php", true);
    xml.onload = () => {
        if (xml.readyState === XMLHttpRequest.DONE) {
            if (xml.status === 200) {
                let data = xml.response;
...
$(document).ready(() => {
    let xml = new XMLHttpRequest();
    xml.open("POST", "/e-diary/student/serverControllers/teacherControllers/userList.php", true);
    xml.onload = () => {
        if (xml.readyState === XMLHttpRequest.DONE) {
            if (xml.status === 200) {
                let data = xml.response;
    ...
    if (sessionStorage.getItem('teacher-id') != null) {
        sendUserLinkId(sessionStorage.getItem('teacher-id'));
        sessionStorage.removeItem('teacher-id');
        if ($(window).width() <= 1000) {
            $(".msg-box").css('display', 'block');
            $(".user-box").css('display', 'none');
        }
